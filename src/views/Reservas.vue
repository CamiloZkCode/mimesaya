    ...
    <section class="filters" aria-label="Filtros de búsqueda">
      <div class="filters__row">
        ...
        <div class="field">
          ...
          <select id="f-hora" v-model="filters.time">
      ...
      <ul class="cards" v-if="filteredTables.length">
        <li v-for="t in filteredTables" :key="t.id" class="card">
          <article>
            ...
            <dl class="card__info">
              <div class="row">
                <dt>Capacidad</dt><dd>{{ t.capacity }} pers.</dd>
              </div>
              <div class="row">
                <dt>Ubicación</dt><dd>{{ t.position }}</dd>
              </div>
              <div class="row">
                <dt>Fecha</dt><dd>{{ displayDate }}</dd>
              </div>
              <div class="row">
                <dt>Hora</dt><dd>{{ filters.time || 'A convenir' }}</dd>
              </div>
              <div class="row" v-if="t.features?.length">
                <dt>Extras</dt><dd>{{ t.features.join(' • ') }}</dd>
              </div>
            </dl>

            <footer class="card__actions">
              <button class="btn btn--primary" @click="openModal(t)">Reservar</button>
              <button class="btn btn--ghost" @click="markFavorite(t)">{{ t.favorite ? 'Quitar favorito' : 'Favorito' }}</button>
            </footer>
          </article>
        </li>
      </ul>
        ...
        >
          ...
          <section class="modal__body">
            <div class="modal__details">
              ...
              <p><strong>Hora:</strong> {{ filters.time || 'A convenir' }}</p>
            </div>
            ...
            <form class="form" @submit.prevent="submitReservation">
              <div class="grid">
                ...
                <div class="field" style="grid-column: 1 / -1;">
                  ...
                  <textarea id="r-notas" v-model.trim="form.notes" rows="3" placeholder="Ej.: silla para bebé, pastel sorpresa, alérgenos..."></textarea>
                </div>
...
const filteredTables = computed(() =>
  tables.value.filter(t => {
    ...
    // fecha/hora: aquí solo se reflejan visualmente; la lógica real sería del backend
    return true
  })
)
const filtersSummary = computed(() => {
  ...
  if (applied.time) parts.push(applied.time + ' h')
  return parts.join(' · ')
})
...
function submitReservation() {
  // Validación simple
  if (!form.name || !form.phone || !form.email || !form.acceptsPolicy) return
  // Aquí dispararías una acción/emit o navegación a “confirmación”
  const payload = {
    tableId: modal.table?.id,
    tableNumber: modal.table?.number,
    place: modal.table?.place,
    date: applied.date,
    time: applied.time || 'A convenir',
...
/* ---------- Extras UI ---------- */
function markFavorite(t: Table) { t.favorite = !t.favorite }
...
/* ---------- UX: bloquear scroll al abrir modal con teclado ESC ---------- */
...
// Inicial: aplicar filtros para reflejar hoy
applyFilters()
</script>

<style scoped>
/* ===== Layout base ===== */
.reservas { display: grid; gap: 1.25rem; padding: 1rem; }
...
.field label { color: var(--color-oscuro); font-weight: 700; font-size: .95rem; }
...
.filters__actions {
  grid-column: span 12; display: flex; gap: .6rem; align-items: end; justify-content: flex-start;
}
.filters__meta { color: var(--color-dark-variant); font-size: .95rem; }

@media (min-width: 720px) {
  #f-lugar.field, #f-personas.field, #f-fecha.field, #f-hora.field {}
  #f-lugar, #f-personas, #f-fecha, #f-hora {}
  .field:nth-child(1){ grid-column: span 3; }
  .field:nth-child(2){ grid-column: span 3; }
  .field:nth-child(3){ grid-column: span 3; }
  .field:nth-child(4){ grid-column: span 3; }
  .filters__actions { grid-column: span 12; justify-content: flex-end; }
}
...
.card > article {
  ...
  box-shadow: var(--box-shadow);
  overflow: hidden;
  display: grid; grid-template-rows: auto 160px auto auto;
}
.card__head {
  display: flex; justify-content: space-between; align-items: center;
  padding: .8rem 1rem .5rem;
}
.card__title { margin: 0; color: var(--color-oscuro); font-size: 1.05rem; }
.badge {
  padding: .25rem .5rem; border-radius: 999px;
  background: linear-gradient(135deg, var(--color-azul-1), var(--color-primary-variant));
  color: var(--color-blanco); font-weight: 700; font-size: .75rem;
}
...
.card__info { padding: .75rem 1rem; display: grid; gap: .35rem; }
.card__info .row { display: grid; grid-template-columns: 1fr auto; gap: .5rem; }
.card__info dt { color: var(--color-dark-variant); }
.card__info dd { margin: 0; color: var(--color-oscuro); font-weight: 600; }

.card__actions {
  display: flex; gap: .6rem; padding: .8rem 1rem 1rem; justify-content: flex-end;
}
...
.modal__header {
  display: flex; justify-content: space-between; align-items: center;
  padding: .9rem 1rem; border-bottom: 1px solid var(--color-info-luz);
}
.modal__header h3 { margin: 0; color: var(--color-oscuro); }
.modal__close {
  background: transparent; border: 1px solid var(--color-info-luz);
  border-radius: var(--border-radius-2);
...
@media (max-width: 560px){ .modal__details { grid-template-columns: 1fr; } }

.form .grid {
  display: grid; gap: .75rem;
  grid-template-columns: repeat(12, 1fr);
}
.form .field { grid-column: span 12; display: grid; gap: .25rem; }
.form .field label { color: var(--color-oscuro); font-weight: 700; }
.form input, .form select, .form textarea {
  border: 1px solid var(--color-info-luz);
  border-radius: var(--border-radius-2);
  padding: .55rem .7rem; font: inherit; background: var(--color-blanco);
}
@media (min-width: 720px){
  .form .field:nth-child(1){ grid-column: span 6; }
  .form .field:nth-child(2){ grid-column: span 6; }
  .form .field:nth-child(3){ grid-column: span 6; }
  .form .field:nth-child(4){ grid-column: span 6; }
}
.checks { display: flex; gap: 1rem; flex-wrap: wrap; }
.chk { display: inline-flex; gap: .4rem; align-items: center; }

.modal__actions {
  display: flex; justify-content: flex-end; gap: .6rem; padding-top: .5rem;
}
